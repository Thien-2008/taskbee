...
function DashboardTab({ user, displayName, balance, tasksDone, onNavigate }: { user: any; displayName: string; balance: number; tasksDone: number; onNavigate: (t: any) => void }) {
    ...
    <div style={{ padding: 20, maxWidth: 800, margin: '0 auto' }}>
      ...
      {isNewUser && (
        <div style={{ background: '#161618', border: '1px solid #1C1C1E', borderRadius: 12, padding: 20, marginBottom: 20 }}>
          ...
          {[{ done: true, text: 'Đăng ký tài khoản' }, { done: false, text: 'Làm task đầu tiên', action: 'Làm ngay', tab: 'tasks' }, { done: false, text: 'Rút tiền đầu tiên', action: 'Xem hướng dẫn', tab: 'withdraw' }].map((step, i) => (
...
function TasksTab({ user, onStartTask }: { user: any; onStartTask: (b: any) => void }) {
  const [batches, setBatches] = useState<any[]>([]); const [loading, setLoading] = useState(true)
  ...
  const reward = (b: any) => Math.round(b.budget_total / b.total_items)
  return (
    <div style={{ padding: 20, maxWidth: 800, margin: '0 auto' }}>
      <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600, fontSize: 20, marginBottom: 16 }}>📋 Danh sách task</h2>
      {loading ? <div style={{ textAlign: 'center', padding: 40, color: '#8A857D' }}>⏳ Đang tải...</div> : batches.length === 0 ? <div style={{ textAlign: 'center', padding: 40, background: '#161618', borderRadius: 12 }}><div style={{ fontSize: 40, marginBottom: 12 }}>📋</div><div style={{ color: '#8A857D' }}>Chưa có task.</div></div> :
...
function ProfileTab({ user, displayName, balance, tasksDone, userRole, onLogout, onNavigate }: { user: any; displayName: string; balance: number; tasksDone: number; userRole: string; onLogout: () => void; onNavigate: (v: string) => void }) {
  return (
    <div style={{ padding: 20, maxWidth: 800, margin: '0 auto' }}>
      <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600, fontSize: 20, marginBottom: 20 }}>👤 Hồ sơ</h2>
      <div style={{ background: '#161618', border: '1px solid #1C1C1E', borderRadius: 12, padding: 20, marginBottom: 20, textAlign: 'center' }}>
