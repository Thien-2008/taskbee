...
export default function Home() {
    ...
    // Canvas lục giác nền (giữ nguyên)
    useEffect(() => {
    ...
    return (
        <div style={{
            ...
            <nav className="nav-glass" style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 24px' }}>
                <a href="#" style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 20, color: '#F5A623', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 8, userSelect: 'none' }}>
                    🐝 TaskBee
                </a>
            ...
            <section style={{ position: 'relative', zIndex: 10, maxWidth: 800, margin: '0 auto', padding: '60px 16px' }}>
                ...
                <div className="calc-grid" style={{ background: '#161618', border: '1px solid #1C1C1E', borderRadius: 20, padding: '32px 28px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, alignItems: 'center' }}>
                    <div>
                        ...
                        <input type="range" min="1" max="8" value={hours} onChange={e => setHours(+e.target.value)}
                            style={{ width: '100%', height: 4, background: 'rgba(245,166,35,0.2)', borderRadius: 2, outline: 'none', marginBottom: 16, WebkitAppearance: 'none' }} />
            ...
            <section style={{ position: 'relative', zIndex: 10, maxWidth: 700, margin: '50px auto', background: 'linear-gradient(135deg, #1a1508, #0f0e0c)', border: '1px solid rgba(245,166,35,0.2)', borderRadius: 24, padding: '60px 24px', textAlign: 'center' }}>
                ...
                <button className="btn-primary" style={{ fontSize: 16, padding: '16px 40px' }}>Đăng ký miễn phí →</button>
