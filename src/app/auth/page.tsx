...
function AuthForm() {
  ...
  // Khi load trang login, kiểm tra localStorage và tự động điền email + tick checkbox
  useEffect(() => {
    if (mode === 'login') {
      const savedEmail = localStorage.getItem('taskbee_email')
      if (savedEmail) {
        setEmail(savedEmail)
        setRememberEmail(true)
      }
    } else {
      // Khi ở mode khác, xóa dữ liệu để không hiển thị email cũ
      setEmail('')
  ...
  // Xử lý khi email input thay đổi -> nếu checkbox đang tick, cập nhật localStorage
  const handleEmailChange = (value: string) => {
    setEmail(value)
    if (rememberEmail && value.trim()) {
      localStorage.setItem('taskbee_email', value.trim().toLowerCase())
    } else if (!value.trim()) {
      // Nếu xóa hết email, xóa localStorage và bỏ tick
      localStorage.removeItem('taskbee_email')
      setRememberEmail(false)
    }
  }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) { setError(translateError(error.message)); setShake(true); setTimeout(() => setShake(false), 600) }
      ...
      // Lưu email vào localStorage nếu checkbox được tick
          ...
          {mode === 'login' && (
            <motion.div key="login" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.25 }}>
              ...
              <form onSubmit={handleLogin} className="space-y-6">
                <div className={fieldBorder}><input type="email" required maxLength={254} value={email} onChange={e => handleEmailChange(e.target.value)} placeholder="Email" className={inputClass} aria-describedby={error ? "auth-error" : undefined} /></div>
