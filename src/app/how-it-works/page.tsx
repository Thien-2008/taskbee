    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: '#0a0a0b', color: '#EDEBE7', minHeight: '100vh', userSelect: 'none' }}>
      ...
      <section style={{ maxWidth: 800, margin: '0 auto', padding: '20px 20px 80px' }}>
        {steps.map((step, idx) => (
          <ScrollReveal key={idx} delay={idx * 0.1}>
            <div style={{ display: 'flex', gap: 16, marginBottom: 40, background: '#161618', border: '1px solid #1C1C1E', borderRadius: 16, padding: '24px 20px', alignItems: 'flex-start' }}>
              <div style={{ width: 48, height: 48, background: 'rgba(245,166,35,0.1)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <section style={{ textAlign: 'center', padding: '40px 20px 60px', maxWidth: 500, margin: '0 auto' }}>
        <ScrollReveal delay={0.2}>
          <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 24, marginBottom: 16 }}>Đã hiểu quy trình? Bắt đầu ngay.</h2>
          <button onClick={() => router.push('/auth?mode=register')} className="btn-primary">Tạo tài khoản miễn phí</button>
        </ScrollReveal>
      </section>
      <Footer />
    </div>
